...
struct AboutView: View {
    ...
    private var versionLabel: String {
        let isPt = Locale.current.language.languageCode?.identifier == "pt"
        let prefix = isPt ? "Versão" : "Version"
        return build.isEmpty || build == version ? "\(prefix) \(version)" : "\(prefix) \(version) (\(build))"
    }
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            // ── Author ───────────────────────────────────────────────────────
            Text(Locale.current.language.languageCode?.identifier == "pt" ? "Desenvolvido por" : "Developed by")
                .font(.footnote)
                .foregroundStyle(.secondary)
